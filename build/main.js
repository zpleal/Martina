...
function restoreSelections() {
    ...
    selectedGame = $(selections.values['panel']);
    if (selectedGame) selectedGame.className += " selected";
}
...
function populateOn(parent, panelStruct, level_) {
    const level = level_ ? level_ : 0; //
    for (const label in panelStruct) {
        ...
        const item = document.createTextNode(label);

        //element.label = label;
        parent.appendChild(element);
        element.appendChild(item);
        element.style.paddingLeft = level + "em"; // 
        element.className = isLeaf ? "option" : "optgroup";
        element.id = value;
        element.onclick = event => {
            parent.value = value;

            if (selectedGame != null) selectedGame.className = "option";

            element.className += " selected";
            selectedGame = element;
        };

        if (isLeaf) element.value = value;else populateOn(parent, value, level + 1); // populateOn(element,value);
    }
}
